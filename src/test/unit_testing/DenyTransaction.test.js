...
describe("FrontierMultisig", function () {
  ...
  it("Should deposit Ether, submit a transaction, deny it and check if it's in completed transactions", async function () {
    ...
    const contractBalance = await ethers.provider.getBalance(frontierMultisig.address);
    ...
    await frontierMultisig.connect(owner).denyTransaction(0);

    // Get the denial count
    const denialCount = await frontierMultisig.getTransactionDenialCount(0);

    // Check if the transaction has 1 denial (from the owner)
    expect(denialCount).to.equal(1);
    ...
    expect(completedTransactions[3][0]).to.equal(false); // executed
    expect(completedTransactions[4][0]).to.equal(true); // denied
